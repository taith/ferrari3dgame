...
public class TestAI extends JPanel implements Animatable, KeyListener {
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_LEFT) { camera.add(move, 0f, 0f); }
		if (e.getKeyCode() == KeyEvent.VK_RIGHT) { camera.add(-move, 0f, 0f); }
		if (e.getKeyCode() == KeyEvent.VK_UP) { camera.add(0f, 0f, move); }
		if (e.getKeyCode() == KeyEvent.VK_DOWN) { camera.add(0f, 0f, -move); }
	}
	...
	private static class TestCircuitData implements CircuitData {
		...
		public TestCircuitData(ResourceFile xml) throws Exception {
			...
			for (Vector3D i : grid) { i.setVector(i.getX() * 10f, i.getY() * 10f, i.getZ() * 10f); } //TODO
		}
