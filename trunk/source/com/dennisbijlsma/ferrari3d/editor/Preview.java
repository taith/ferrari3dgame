	...
	protected void init() {
		...
		SwingGame game = new SwingGame() {
			public void initGameState() {
				...
				getContext().getSceneGraph().getRootNode().addChild(root);	
				
				Camera camera = new Camera(getContext().getDisplay());			
				camera.aim(new Vector3D(0f, 10f, -100f), new Vector3D());
				getContext().getSceneGraph().addCamera(camera);
				
				LightSource light = new LightSource(LightSource.LightType.DIRECTIONAL,
						new Vector3D(0f, -1f, 0.5f), new Color(200, 200, 200), Color.WHITE);
				getContext().getSceneGraph().addLight(light);
			}
						
			public void updateGameState(float dt) {			
				if (rootNode != null) {
					updateControls(getContext().getController(), getContext().getSceneGraph().getCamera(0));
