...
public class Preview extends JPanel {
	...
	private SceneGraphGroupNode root;
	private SceneGraphGroupNode rootNode;
	...
	protected void init() {
		...
		final SwingGame game = new SwingGame(500, 500, 25) {
			public void initGameState() {
				root = new SceneGraphGroupNode("__preview");
			...
			public void updateGameState(float dt) {			
				if (rootNode != null) {
					...
					rootNode.getTransform().getRotation().setX(viewAngleX);
					rootNode.getTransform().getRotation().setY(viewAngleY);
					if (spinning) { 
						rootNode.getTransform().getRotation().add(0f, SPIN_SPEED, 0f); 
					}
	...
	private void setSpinning() {
		if ((spinning) || (rootNode == null)) {
			...
			spinning = true;
			rootNode.getTransform().getRotation().setY(0f);
		}	
	}
