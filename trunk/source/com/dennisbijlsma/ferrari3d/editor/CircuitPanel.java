...
public class CircuitPanel extends FormPanel {
	...
	public CircuitPanel(Preview preview) {
		...
		editButton.setToolTipText(settings.getText("editor.circuit.editbuttontooltip"));
	...
	private void remove() {
		...
		if (mode==MODE_POINTS) { file.points.remove(getSelectedPoint()); }
		if (mode==MODE_GRID) { file.startgrid.remove(getSelectedGrid()); }
		if (mode==MODE_CAMERAS) { file.circuitCameras.remove(getSelectedCamera()); }
	...
	private void edit(KeyEvent e) {
		...
		if (mode==MODE_POINTS) { editPoint(index,getSelectedPoint(),delta,e); }
		if (mode==MODE_GRID) { editGrid(index,getSelectedGrid(),delta); }
		if (mode==MODE_CAMERAS) { editCamera(index,getSelectedCamera(),delta); }
	}
	...
	private void editPoint(int index,CircuitPoint p,Vector3D delta,KeyEvent e) {
		...
		if (e.getKeyCode()==KeyEvent.VK_E) {
			...
			String temp=PopUp.input(getTopLevelAncestor(),label,""+p.speed);
			try {
