...
public class Editor extends JFrame implements ActionListener {
	...
	protected void createFile(File fileURL) {
		...
		file.setMeta(F3dFile.META_DATE,TextFormatter.dateFormat(false));
	...
	private File showFileDialog(boolean saveMode) {
		...
		dialog.addIcon("f3d",new ImageIcon(Settings.ICON));
	...
	private File showImportDialog() {
		...
		importList.putClientProperty(MacHandler.QUAQUA_LIST,"striped");
	...
	private void editMenu() {
		...
		try {
			contents=DataLoader.loadString(new FileInputStream(source),"UTF-8");
		} catch (IOException e) {
	...
	private void setMenuItem(JMenuItem item,String label,JMenu menu,int key,boolean shift) {
		...
		if (key!=-1) {
			int mask=Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()+(shift ? KeyEvent.SHIFT_DOWN_MASK : 0);
