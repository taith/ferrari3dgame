...
public class F3dLoader {
	...
	private HashMap<String,Document> xmlCache;	
	private HashMap<Image,String> textureURLs;
	...
	public F3dFile load(String location,boolean local,boolean share) throws Exception {
		...
		file.meta.put(F3dFile.META_AUTHOR,XMLParser.getChildValue(metaNode,F3dFile.META_AUTHOR));
		file.meta.put(F3dFile.META_DATE,XMLParser.getChildValue(metaNode,F3dFile.META_DATE));
		file.meta.put(F3dFile.META_VERSION,XMLParser.getChildValue(metaNode,F3dFile.META_VERSION));
		file.meta.put(F3dFile.META_PREVIEW,XMLParser.getChildValue(metaNode,F3dFile.META_PREVIEW));
		
		if (!file.meta.get(F3dFile.META_VERSION).equals(Editor.VERSION)) {
			Settings.getInstance().getLog().info("Data file '"+location+"' has different version");
		}
		...
		for (Node i : XMLParser.getChildNodes(XMLParser.getFirstChildNode(circuitNode,"points"))) {
			file.points.add(parseCircuitPoint(i,file.meta.get(F3dFile.META_VERSION)));
		}
	...
	private SceneGraphNode parseSceneGraphNode(Node node,String prefix,boolean share) throws IOException {
		...
		if (type.equals("model")) { 
			result=getModel(Resources.getResource(prefix+XMLParser.getChildValue(node,"url")),share);
			result.setName(id);
			Material material=parseMaterial(XMLParser.getFirstChildNode(node,"material"),prefix);
			if (material!=null) { ((Model) result).setMaterial(material); }
		}
		
		if (type.equals("basicmodel")) { 
			...
			result=new BasicModel(id,shapeType,dimensions);
			Material material=parseMaterial(XMLParser.getFirstChildNode(node,"material"),prefix);
			if (material!=null) { ((BasicModel) result).setMaterial(material); }
		}
		
		if (type.equals("billboard")) {
			Vector3D dimensions=parseVector3D(XMLParser.getFirstChildNode(node,"dimensions"));
			Material material=parseMaterial(XMLParser.getFirstChildNode(node,"material"),prefix);
			BasicModel quad=new BasicModel("billboardQuad",BasicModel.TYPE_QUAD,dimensions);
	...
	private CircuitPoint parseCircuitPoint(Node node,String version) {
	...
	/**
	 * Parses a <code>Material</code> object from XML.
	 * @param node The XML node to parse.
	 * @param texdir The location where texture images are located.
	 * @return The created <code>Material</code>.
	 */
	
	private Material parseMaterial(Node node,String texdir) {
	
		if ((node==null) || (XMLParser.getNodeValue(node).equals("none"))) {
			return null;
		}
		...
		if (!temp[0].equals("none")) { m.setColor(Utils.getColor(temp[0])); }		
		if (!temp[1].equals("none")) { m.setAlpha(Integer.parseInt(temp[1])); }
		if (!temp[3].equals("none")) { m.setWireframe(temp[3].equals("true")); }
		
		if (!temp[2].equals("none")) {
			Image texture=getTexture(Resources.getResource(texdir+temp[2]));
			m.setTexture(texture);
			textureURLs.put(texture,temp[2]);
		}		
		
		if (!temp[4].equals("none")) {
			Image bumpmap=getTexture(Resources.getResource(texdir+temp[4]));
			m.setBumpMap(bumpmap);
			textureURLs.put(bumpmap,temp[4]);
		}
		
		if ((temp.length>5) && (!temp[5].equals("none"))) {
			Image envmap=getTexture(Resources.getResource(texdir+temp[5]));
			m.setEnvironmentMap(envmap);
			textureURLs.put(envmap,temp[5]);
		}
		
		if ((temp.length>6) && (!temp[6].equals("none"))) {
			m.setTextureBlend(temp[6].equals("true"));
		}
	...
	public void save(F3dFile file) throws Exception {
		...
		writer.println("\t<meta>");
		writer.println(printTag(F3dFile.META_AUTHOR,file.meta.get(F3dFile.META_AUTHOR),2));
		writer.println(printTag(F3dFile.META_DATE,file.meta.get(F3dFile.META_DATE),2));
		writer.println(printTag(F3dFile.META_VERSION,file.meta.get(F3dFile.META_VERSION),2));
		writer.println(printTag(F3dFile.META_PREVIEW,file.meta.get(F3dFile.META_PREVIEW),2));
		writer.println("\t</meta>");
	...
	private void printSceneGraphNode(PrintWriter writer,SceneGraphNode node) {
		...
		if (node instanceof BasicModel) { type="basicmodel"; }
		...
		writer.println(printTag("scale",""+node.getTransform().getScale(),3));
		if (type.equals("model")) {
			Model model=(Model) node;
			writer.println(printTag("url",new File(model.getGeometryResource()).getName(),3));
			writer.println(printTag("material",printMaterial(model.getMaterial()),3));
			writer.println(printTag("shader",printShader(model.getShader()),3));
		}
		if (type.equals("basicmodel")) {
			BasicModel basic=(BasicModel) node; 
			writer.println(printTag("shapetype",""+basic.getType(),3));
			writer.println(printTag("dimensions",printVector3D(basic.getDimensions()),3));
			writer.println(printTag("material",printMaterial(basic.getMaterial()),3));
			writer.println(printTag("shader",printShader(basic.getShader()),3));
		}
		if (type.equals("billboard")) {
			Billboard billboard=(Billboard) node;
			writer.println(printTag("dimensions",printVector3D(billboard.getModel().getDimensions()),3));
			writer.println(printTag("material",printMaterial(billboard.getModel().getMaterial()),3));
		}
		writer.println("\t\t</node>");
	}
	...
	/**
	 * Prints a <code>Material</code> node as XML. The XML is appended to the 
	 * specified <code>PrintWriter</code>.
	 * @param writer The <code>PrintWriter</code> to write XML to.
	 * @param m The object to convert to XML. 
	 */
	
	private String printMaterial(Material m) {
		
		if (m==null) {
			return "none";
		}
		
		String textureURL=(m.getTexture()!=null) ? textureURLs.get(m.getTexture()) : "none";
		String bumpmapURL=(m.getBumpMap()!=null) ? textureURLs.get(m.getBumpMap()) : "none";
		String envmapURL=(m.getEnvironmentMap()!=null) ? textureURLs.get(m.getEnvironmentMap()) : "none";
		
		return (m.getColor()!=null ? Utils.getColor(m.getColor()) : "none")+","+m.getAlpha()+
				","+textureURL+","+m.getWireframe()+","+bumpmapURL+","+envmapURL+","+m.getTextureBlend();
	}
