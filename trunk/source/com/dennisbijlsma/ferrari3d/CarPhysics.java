 ...
 * be called every frame in order to produce accurate results.<p>
...
public class CarPhysics {
	...
	public void updatePhysics(float dt) {
		...
		position.x = (float) (position.x - Math.sin(direction) * speed * SCALE);
		position.y = 0f;
		position.z = (float) (position.z - Math.cos(direction) * speed * SCALE);
		
		orientation = orientation - angularSpeed;
