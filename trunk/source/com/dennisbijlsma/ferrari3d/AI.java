...
public class AI extends Contestant {
	...
	protected static enum RaceLine { 
		IDEAL,
		OVERTAKING 
	}
	...
	protected void updateAI(float dt) {
		...
		Behavior newBehavior=Behavior.DRIVING;
		...
		for (Contestant i : getSession().getContestants()) {
			...
			if ((behaviorTime>0f) && (isLineFront(tempVector2))) { 
				newBehavior=Behavior.OVERTAKING; 
			}
			
			if ((behaviorTime>0f) && (isLineBack(tempVector2))) { 
				newBehavior=Behavior.BLOCKING; 
			}
			
			if (isMinimumDistance(tempVector2)) {
				if (line==RaceLine.IDEAL) {
					setRaceLine(RaceLine.OVERTAKING);
				} else {
					setControl(Controls.ACCELERATOR,0);
				}
			}
		...
		if (behaviorTime<=0f) {
			...
			switch (behavior) {
				case DRIVING : behaviorTime=0f; setRaceLine(RaceLine.IDEAL); break;
				case OVERTAKING : behaviorTime=getOvertakingBehaviorTime(); setRaceLine(getOvertakingLine()); break;
				case BLOCKING : behaviorTime=getBlockingBehaviorTime(); setRaceLine(getBlockingLine()); break;
				default : break;
			}
		} else {
			behaviorTime-=dt;
	...
	/**
	 * Sets the new behavior for this AI car. 
	 */
	
	protected void setBehavior(Behavior newBehavior) {
	
		if ((behavior==newBehavior) || (newBehavior==null)) {
			return;
		}
	...
	/**
	 * Returns the current behavior for this AI car.
	 */
	...
	protected void setRaceLine(RaceLine newLine) {
		
		if ((line==newLine) || (newLine==null)) {
			return;
		}
	...
	/**
	 * Returns the current racing line for this AI car.
	 */
	
	protected RaceLine getRaceLine() {
	...
	private RaceLine getOvertakingLine() {
	
		return (line==RaceLine.IDEAL) ? RaceLine.OVERTAKING : RaceLine.IDEAL;
	}
	...
	private RaceLine getBlockingLine() {
