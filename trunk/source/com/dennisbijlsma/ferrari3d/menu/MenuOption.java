...
public class MenuOption extends MenuWidget {
	...
	public MenuOption(final String label,String value,int numButtons) {
		...
		if (numButtons==1) {
			editButton=new MenuButton(Settings.getInstance().getText("menu.edit")) {
				public void pressed() {
					String input=Popups.inputMessage(null,label,getValue());
					if (input!=null) {
						editPressed(input);
					}
				}
	...
	public void plusPressed() {
		...
		for (ActionListener i : listeners) {
			i.actionPerformed(new ActionEvent(ACTION_PLUS,ActionEvent.ACTION_PERFORMED,"plus"));
		}
	}
	...
	public void minusPressed() {
		...
		if (listeners.size()>0) {
			for (ActionListener i : listeners) {
				i.actionPerformed(new ActionEvent(ACTION_MINUS,ActionEvent.ACTION_PERFORMED,"minus"));
			}
		} else {
			if ((instance!=null) && (field!=null) && (datatype.equals("boolean"))) {
				setSetting(""+!getSetting().equals("true"));
	...
	public void editPressed(String input) {
		...
		if (listeners.size()>0) {
			for (ActionListener i : listeners) {
				i.actionPerformed(new ActionEvent(ACTION_EDIT,ActionEvent.ACTION_PERFORMED,"edit"));
			}
		} else {
	...
	/**
	 * Adds the specified <code>ActionListener</code> to be noified when any of
	 * the buttons within this widget are pressed.
	 */
