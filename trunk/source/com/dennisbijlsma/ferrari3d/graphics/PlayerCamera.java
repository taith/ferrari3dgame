...
public class PlayerCamera {
	...
	public void update() {
		...
		if ((offsetV == 0f) && (offsetH == 0f)) {
			...
			float angle = target.getModel().getTransform().getRotation().getY();
			Vector3D d = new Vector3D();
			d.setX((float) (targetPosition.getX() + Math.cos(angle + offsetH) * selectedCamera.getX()));
			d.setY(selectedCamera.getY());
			d.setZ((float) (targetPosition.getZ() + Math.sin(angle + offsetH) * selectedCamera.getX()));
