...
public class Car extends AbstractCar {
	...
	public Car(SceneGraphGroupNode geometry, LODNode lodNode, SceneGraphNode circuitNode) {
		...
		shadow.getTransform().getRotation().setY(1.57f);
	...
	private float pickCircuit() {
		...
		pickVector.setVector(position.getX(), PICK_HEIGHT, position.getZ());
	...
	private float pickCircuitFromTyre(Model tyre) {
		...
		pickVector.setVector(worldPosition.getX(), PICK_HEIGHT, worldPosition.getZ());		
		return circuitNode.pickDistance(pickVector, PICK_DIRECTION_FLOOR, true);
	}
	...
	public void updatePhysics(float dt, float speed) {
		...
		geometryRotation.setX(longitudinal);
		geometryRotation.setZ(lateral);
		shadow.getTransform().getPosition().setY(referenceY - position.getY() + SHADOW_OFFSET);
		
		// Gravity
		
		isOnFloor = (Math.abs(position.getY() - referenceY) < EPSILON);
		position.setY(referenceY);
	}
	...
	public void updateWheels(float dt, float speed, int steering) {
		...
		tyreLF.getTransform().getRotation().add(wheelSpeedLong, 0f, 0f);
		tyreRF.getTransform().getRotation().add(wheelSpeedLong, 0f, 0f);
		tyreLR.getTransform().getRotation().add(wheelSpeedLong, 0f, 0f);
		tyreRR.getTransform().getRotation().add(wheelSpeedLong, 0f, 0f);
		
		// Lateral
		
		float wheelLateral = -Math.signum(steering) * WHEEL_LATERAL;
		tyreLF.getTransform().getRotation().setY(wheelLateral);
		tyreRF.getTransform().getRotation().setY(wheelLateral);
		helmet.getTransform().getRotation().setY(wheelLateral);
	}
	...
	public void updateEffects(float dt, float speed, boolean braking, int steering) {
		...
		if (braking && (speed > BRAKING_CAMBER_SPEED)) {
			model.getTransform().getPosition().add(0f, -BRAKING_CAMBER_POS, 0f);
			geometry.getTransform().getRotation().setX(BRAKING_CAMBER_ROT);
			helmet.getTransform().getRotation().setX(BRAKING_CAMBER_ROT * 2f);
		}
		...
		if (!isOnTrack) {
			...
			geometry.getTransform().getRotation().setX(wobble);
		}
		
		// Steering wheel
		
		float steerOffset = STEERING_WHEEL_OFFSET * -Math.signum(steering);
		steeringWheel.getTransform().getRotation().setZ(steerOffset);
	}
	...
	public boolean isCollidingObject(float newX, float newY, float newZ) {
		...
		Vector3D position = model.getTransform().getPosition();
		pickVector.setVector(position.getX(), position.getY() + COLLISION_HEIGHT, position.getZ());
		pickVectorDir.setVector(newX - position.getX(), 0f, newZ - position.getZ());
