...
public class Circuit implements CircuitData {
	...
	public ImmutableVector3D getClosestCamera(ImmutableVector3D v) {
		...
		float closestDistance = Utils.getDistance(closest.x, closest.z, v.getX(), v.getZ());
	
		for (Vector3D i : cameras) {
			if (Utils.getDistance(i.x, i.z, v.getX(), v.getZ()) < closestDistance) {
				closest = i;
				closestDistance = Utils.getDistance(closest.x, closest.z, v.getX(), v.getZ());
			}
		}
