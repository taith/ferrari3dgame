...
public class Loader {
	...
	public static void createWorld(SceneGraph scene,Display viewport,boolean fullDetail) {
		...
		if (!Settings.getInstance().splitscreen) {
			scene.addCamera(new Camera(viewport));
		} else {
			scene.addCamera(new Camera(viewport,Camera.SplitScreen.TOP));
			scene.addCamera(new Camera(viewport,Camera.SplitScreen.BOTTOM));
		}
	...
	public static Car loadCar(String carName,SceneGraph scene,Camera camera) {
		...
		try {
			ResourceFile resource=Resources.getResourceFile("cars/"+carName+"/"+carName+".xml");
	...
	public static Circuit loadCircuit(String circuitName,SceneGraph scene) {
		...
		try {
			ResourceFile resource=Resources.getResourceFile("circuits/"+circuitName+"/"+circuitName+".xml");
	...
	public static Circuit createTestCircuit() {
		...
		Circuit circuit=new Circuit("test",circuitNode,points,new Vector3D[0],new Vector3D[0]);		
		return circuit;
	}
