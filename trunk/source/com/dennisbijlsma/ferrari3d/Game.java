...
public class Game implements GameState, GameEventListener {
	...
	private void updateControls(float dt) {
		...
		if (controller.isKeyReleased(CONTROL_REPLAY)) {
			if (!settings.splitscreen && (multiplayer.getNumConnections() == 0)) { 
				isReplay = true;
	...
	private void updateReplay(float dt) {
		...
		for (Contestant i : replayData.keySet()) {
			...
			i.getCar().getTransform().setPosition(new Vector3D(data[0], data[1], data[2]));
			i.getCar().getTransform().getRotation().setY(data[3]);
		}
	}
