...
import com.dennisbijlsma.core3d.game.InitDisplayException;
...
public class Game extends GameCore implements GameEventListener {
	...
	public void initGame() {
		...
		if (settings.aiActive) {
			for (int i=session.getNumContestants(); i<Settings.MAX_CONTESTANTS; i++) {
				...
				Car aiCar=Loader.loadCar(aiCarName,getSceneGraph(),mainCamera);
		...
		getSceneGraph().addOverlay(hud,getSceneGraph().getCamera(0));
		
		if (settings.splitscreen) {
			hud2=new HUD(getDisplay(),getSceneGraph().getCamera(1),session);
			getSceneGraph().addOverlay(hud2,getSceneGraph().getCamera(1));
		}
	...
	private void updateGameLogic(float dt) {
		...
		if (hud2!=null) {
			...
			hud2.setGameData(getCurrentFPS(),getCurrentUPS(),0,startTimer,finishTimer);
		}
	}
