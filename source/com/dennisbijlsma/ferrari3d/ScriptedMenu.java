...
import com.dennisbijlsma.ferrari3d.menu.*;
...
public class ScriptedMenu extends JPanel {
	...
	private HashMap<String,MenuScreen> menus;
	...
	protected void createMenu(String id) {
		...
		String file=DataLoader.loadTextFile(Resources.getResource("data/menu/"+id+".bsh"),"UTF-8");
	...
	public void doBackground(final String targetMenu,final Runnable r) {
		...
		Thread t=new Thread() {
			public void run() {
				setSelectedMenu(targetMenu);
				menus.get(targetMenu).setLoading(true);
				r.run();
				setSelectedMenu(targetMenu);
				menus.get(targetMenu).setLoading(false);
			}
		};
		t.start();
	}
	...
	/**
	 * Logs the specified script error. The exception will be formatted so that
	 * the stack trace of the script itself can also be found.
	 */
	
	private void log(EvalError e,String filename) {
		
		String source="("+filename+":"+e.getErrorLineNumber()+")";
		String stacktrace=e.getErrorText();
		settings.getLog().warning("Error in script file '"+filename+"'\n"+e.getMessage()+" "+source+"\n"+stacktrace);
	}
		...
		ConnectionListener listener=new ConnectionListener() {
			public void messageReceived(Connection c,Message message) {
				...
				if (message.getType().equals(Multiplayer.MESSAGE_DISCONNECT)) {
					PopUp.message(null,settings.getTexts().getText("menu.message.clientdisconnect",c.getHost()));
					setSelectedMenu(MENU_MAIN);
				}
	...
	private void showSystemInfo() {
		...
		system+=settings.getText("menu.systeminfo.os")+" "+Resources.getPlatformName()+"\n";
		...
		system+=settings.getText("menu.systeminfo.memory")+" "+Utils.memoryFormat(memory,false)+"\n";
