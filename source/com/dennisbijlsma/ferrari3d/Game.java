...
public class Game extends GameCore implements GameEventListener {
	...
	private static final String CONTROL_SCREENSHOT=InputController.KEY_0;
	...
	protected void initGame() {
		...
		hud=new HUD(viewport,scene,session);
		viewport.addHUD(hud);
	...
	private void updateControls(float dt) {
		...
		if (controller.isKeyReleased(CONTROL_SCREENSHOT)) {
			String screenshotURL="screenshot_"+Utils.getTimestamp();
			getScreenshot(screenshotURL);
			hud.setMessage(settings.getText("game.screenshot")+" '"+screenshotURL+"'");
		}
	...
	private void updateGameLogic(float dt) {
		...
		hud.setGameData(getFramerate(),getPolygons(),startTimer,finishTimer);
	}
	...
	public void onGameEvent(GameEvent event) {
		...
		if (type.equals(Multiplayer.MESSAGE_DISCONNECT)) {
			exit();
			hud.setMessage(settings.getTexts().getText("game.disconnectmessage",id));
		}
	}
