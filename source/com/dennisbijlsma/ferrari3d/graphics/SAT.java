...
public class SAT {
	...
	public void paintDefaultScreen(String driver,String car,int pos) {
		...
		driver=Utils.nameFormat(driver.toUpperCase());
		car=car.toUpperCase();
	...
	public void paintTimeDiffScreen(String driver1,Laptime laptime1,String driver2,
		...
		driver1=Utils.nameFormat(driver1.toUpperCase());
		driver2=Utils.nameFormat(driver2.toUpperCase());
	...
	public void paintStandingsScreen(String[] drivers,Laptime[] laptimes) {
		...
		for (int i=0; i<Math.min(drivers.length,8); i++) {
			String driver=Utils.nameFormat(drivers[i].toUpperCase());
	...
	private void paintBackground() {
		...
		if (backgroundImage==null) {
			backgroundImage=new BufferedImage(area.width,area.height,BufferedImage.TYPE_INT_ARGB);
			Graphics2D imageG2=backgroundImage.createGraphics();
			imageG2.setColor(BACKGROUND_COLOR);
			imageG2.fillRect(0,0,backgroundImage.getWidth(),backgroundImage.getHeight());
			imageG2.dispose();
		}
	...
	private void paintText(String text,Font font,Color color,int x,int y,char align) {
		...
		g2.setFont(font);
		g2.setColor(color);
		Utils.drawAlignedString(g2,text,x,y,align);
	}
	...
	private static String getIntermediateLabel(int intermediate) {
	
		if (intermediate==0) { return "INTERMEDIATE 1"; }
		if (intermediate==1) { return "INTERMEDIATE 2"; }
		if (intermediate==2) { return "START / FINISH"; }
		return "???";
	}
