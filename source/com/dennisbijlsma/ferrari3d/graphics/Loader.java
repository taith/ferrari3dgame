...
public class Loader {
	...
	public static Car loadCar(String carName,SceneGraph scene) {
		...
		try {
			...
			Settings.getInstance().getLog().error("Problem during loading of car file",e);
		}
				
		SceneGraphNode carNode=file.getNode();
		carNode.getTransform().setScale(CAR_SCALE);
		
		BasicModel lod=new BasicModel("lod",BasicModel.TYPE_BOX,LOD_SIZE);
		lod.setMaterial(new Material(LOD_COLOR));
	...
	public static Circuit loadCircuit(String circuitName,SceneGraph scene) {
		...
		try {
			...
			Settings.getInstance().getLog().error("Problem during loading of circuit file",e);
		}
	...
	public static void createIndicator(SceneGraphNode node,String text) {
		...
		Utils.drawAlignedString(g2,text,image.getWidth()/2,25,'c');
		g2.dispose();
		
		BasicModel indicator=new BasicModel("indicator",BasicModel.TYPE_BOX,INDICATOR_SIZE);
		indicator.getTransform().getPosition().y=INDICATOR_HEIGHT;
		indicator.setMaterial(new Material(image));
