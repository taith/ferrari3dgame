...
/**
 * Controls the heads-up-display for the game. The displayed components depend on
 * the selected car and camera. 
 */
...
public class HUD extends Overlay {
	...
	public HUD(Display viewport, Camera camera, Session session) {
		
		super(viewport, camera, 0, viewport.getHeight() - HUD_HEIGHT,
				getWidth(viewport), HUD_HEIGHT);
	...
	private void paintRadar(Graphics2D g2) {
		...
		for (Contestant i : session.getContestants()) {			
			...
			distance.relativeTo(d1, d2, target.getOrientation());
	...
	private void paintSAT(Graphics2D g2) {
		...
		if (fastest.getBestLaptime().getTime() < satBestLap) {
			sat.setScreen(SAT.Screen.FASTEST_LAP, SAT_TIME);
			satBestLap = fastest.getBestLaptime().getTime();
		}
		
		// Paint SAT
		
		ImmutableVector3D position=target.getPosition();
		CircuitPoint line=target.getCircuit().getIntermediate(target.getIntermediate());		
		boolean near=(Utils.getDistance(position.getX(),position.getZ(),line.pointX,line.pointY)<SAT_DISTANCE);		
		...
		if (sat.getScreen()==SAT.Screen.FASTEST_LAP) {
			sat.paintFastestLapScreen(fastest.getName(),fastest.getCar().getCarName(),fastest.getBestLaptime());
			return;
		}
		...
		if (sat.getScreen()==SAT.Screen.STANDINGS) {
			...
			for (int i=0; i<bestNames.length; i++) {
				bestNames[i]=session.getRacePositionID(i+1).getName();
		...
		if ((session.getMode()==Session.SessionMode.TIME) && (near) && (target.getLap()>1)) {
			sat.paintTimeDiffScreen(target.getName(),target.getCurrentLaptime(),leader.getName(),
		...
		if ((session.getMode()==Session.SessionMode.TIME) && (sat.getScreen()==SAT.Screen.TIME_DIFFERENCE)) {
			if (satBestLap!=LapTime.TIME_NOT_SET) {
				...
				sat.paintTimeDiffScreen(target.getName(),targetLap,leader.getName(),leader.getBestLaptime(),
		...
		if ((session.getMode()==Session.SessionMode.RACE) && (sat.getScreen()==SAT.Screen.TIME_DIFFERENCE)) {
			if (inFront!=null) {
				sat.paintPosDiffScreen(inFront.getName(),pos-1,target.getName(),pos,0);
				return;
			}
		}
		...
		if ((session.getMode()==Session.SessionMode.TIME) && (near) && (target.getLap()<2)) { 
			sat.paintDefaultScreen(target.getName(),target.getCar().getCarName());
			return;
		}
	}
	...
	private void paintMessage(Graphics2D g2) {
		...
		g2.fillRoundRect(viewport.getWidth() / 2 - messageWidth / 2 - 10, 0, messageWidth + 20,
	...
	private void paintDebug(Graphics2D g2) {
		...
		g2.drawString(settings.getText("game.memory") + " " + (memory / 100000) / 10f + " mb", 20, 60);
	...
	public void setGameData(float framerate, float ups, long polygons, float startTimer, 
			float finishTimer) {
	...
	/**
	 * Returns the width of this HUD, based on the width of the display.
	 */
