...
import com.dennisbijlsma.core3d.game.*;
...
public class HUD extends ConcurrentSwingHUD {
	...
	public HUD(Viewport viewport,Session session,SceneGraph scene) {
		
		super(0,viewport.getHeight()-HUD_HEIGHT,getWidth(viewport),HUD_HEIGHT,viewport,scene);
		...
		Thread t=new Thread(this,"Ferrari3D-HUD");
		t.start();
	}
	...
	private void paintInfo(Graphics2D g2) {
		...
		if (Settings.getInstance().showFramerate) {
			...
			g2.setFont(DATA_FONT);
			g2.setColor(SHADOW_COLOR);			
			Utils2D.drawAlignedString(g2,fps,right-9,21,'r');
			g2.setColor(FONT_COLOR);			
			Utils2D.drawAlignedString(g2,fps,right-10,20,'r');
		}
	...
	private void paintDebug(Graphics2D g2) {
		...
		g2.drawString("Java "+Resources.getJavaVersion()+" / "+Resources.getPlatformName(),20,75);
	}
