...
public class HUD extends ViewportHUD {
	...
	private static final int SAT_TIME_MESSAGE=50;
	private static final int SAT_TIME_STANDINGS=100;
	...
	public HUD(Viewport viewport,SceneGraph scene,Session session) {
		
		super(viewport,scene,0,viewport.getHeight()-HUD_HEIGHT,getWidth(viewport),HUD_HEIGHT);
	...
	private void paintInfo(Graphics2D g2) {
		...
		g2.drawString(target.getLap()+" / "+Settings.getInstance().laps,right-90,getHeight()-45);
		...
		Utils.drawAlignedString(g2,posText,right-18,getHeight()-73,'r');
		g2.setColor(FONT_COLOR);
		Utils.drawAlignedString(g2,posText,right-20,getHeight()-75,'r');
				
		if (Settings.getInstance().showFramerate) {
			float framerate=(Float) gamedata.get(GAME_DATA_FRAMERATE);
			String fps=settings.getText("game.framerate")+" "+Math.round(framerate)+" fps";
			g2.setFont(DATA_FONT);
			g2.setColor(SHADOW_COLOR);			
			Utils.drawAlignedString(g2,fps,right-9,21,'r');
			g2.setColor(FONT_COLOR);			
			Utils.drawAlignedString(g2,fps,right-10,20,'r');
		}
	...
	private void paintSAT(Graphics2D g2) {
		...
		sat.setGraphics(g2);
		sat.setArea(0,0,viewport.getWidth(),getHeight());
		...
		boolean info=((satTimer==1) && (target.getLap()>1) && (Math.round(Math.random()*2)==0));
		...
		if (info) {
			sat.setTimer(SAT_TIME_STANDINGS);
		}
		...
		if (sat.getTimer()>0) {
			...
			sat.setTimer(sat.getTimer()-1);			
			return;
		}
		
		// Paint time screen
		
		if ((session.getMode()==Session.SessionMode.TIME) && (!near) && (satTimer==0) && (target.getLap()>1)) {
			//sat.paintTimeScreen(target.getID(),target.getCurrentLaptime());
			//return;
		}
		...
		if ((session.getMode()==Session.SessionMode.TIME) && (satTimer>0) && (target.getLap()>1)) {
		...
		if ((session.getMode()==Session.SessionMode.RACE) && (satTimer>0)) {
			if (inFront!=null) {
	...
	private void paintMessage(Graphics2D g2) {
		...
		Utils.drawAlignedString(g2,message,viewport.getWidth()/2,messageHeight,'c');
	}
	...
	private void paintDebug(Graphics2D g2) {
		...
		String arch=System.getProperty("os.arch").indexOf("64")==-1 ? "32 bit" : "64 bit";
		long memory=Utils.getConsumedMemory();
		float framerate=(Float) gamedata.get(GAME_DATA_FRAMERATE);
		long polygons=(Long) gamedata.get(GAME_DATA_POLYGONS);
		
		g2.setColor(MESSAGE_BACKGROUND);
		g2.fillRect(10,0,200,70);
		g2.setColor(MESSAGE_FOREGROUND);
		g2.drawRect(10,0,200,70);
		g2.setFont(DEBUG_FONT);		
		g2.drawString(settings.getText("game.framerate")+" "+Math.round(framerate)+" fps",20,15);
		g2.drawString(settings.getText("game.polygons")+" "+polygons,20,30);
		g2.drawString(settings.getText("game.processors")+" "+processors+" ("+arch+")",20,45);
		g2.drawString(settings.getText("game.memory")+" "+Utils.memoryFormat(memory,false),20,60);
	}
	...
	public void setGameData(float framerate,long polygons,float startTimer,float finishTimer) {
	...
	private static int getWidth(Viewport viewport) {
