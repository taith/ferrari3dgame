...
public class Settings {
	...
	public ArrayList<String> circuits;
	public String[][] controlsets;

	private String version;
	private Log log;
	private LanguageBundle texts;
	private HashMap<String,TrackRecord[]> records;
	...
	public static final String LANGUAGE_KEY_ENGLISH="english";
	...
	public static final String TEXTS_URL="data/texts/texts_english.properties";
	...
	public void init() throws Exception {
		...
		Properties english=DataLoader.loadProperties(Resources.getResourceStream(TEXTS_URL));
		texts.addLanguage(LANGUAGE_KEY_ENGLISH,english);
		
		// Setup log
		
		log=new Log("com.dennisbijlsma.ferrari3d");
		log.setConsole(true);
		...
		if (Utils.isWebstart()) {
			log.setFile(true,Resources.getApplicationData("Ferrari3D","log.txt"));
		}
	}
	...
	public void load() {
		...
		if (!file.exists()) {
			defaults();
			log.info("Recreating settings file at '"+file.getAbsolutePath()+"'");
		}
		...
		try {
			...
			if (!version.equals(p.getProperty("version"))) {
				defaults();
				load();
				log.info("Settings file has incompatible version '"+p.getProperty("version")+"'");
				return;
			}
	...
	public void save() {
		...
		try {
			...
			log.error("Could not save settings file",e);
		}
	}
	...
	public void checkForUpdates() {
		...
		try {
			Properties p=DataLoader.loadProperties(new URL(UPDATE_URL));
	...
	public void sendReport() {
		...
		try {
			...
			log.warning("Could not send report",e);
		}
	...
	protected void sendErrorReport(Throwable exception) {
		...
		String stacktrace=Log.getStackTraceAsString(exception);
		...
		try {
			...
			log.warning("Could not send error report",e);
		}
	}
	...
	/**
	 * Returns the <code>Texts</code> object that contains resource texts. This 
	 * can be used to obtain and format language-specific data.
	 */
	
	public LanguageBundle getTexts() {
		
		return texts;
	}
	
	/**
	 * Returns the text with the specified key. See <code>Texts.getText(String)
	 * </code> for more information.
	 */
	
	public String getText(String key) {
	
		if (texts.getText(key)!=null) {
			return texts.getText(key);
		} else {
			log.info("Unknown text: '"+key+"'");
			return "???";
		}
	}
	...
	public void reloadRecords() {
		...
		try {
			...
			log.warning("Could not load track records",e);
		}
	}
