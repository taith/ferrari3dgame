...
public class ContentLoader {
	...
	private SceneGraphNode parseSceneGraphNode(Element node, String path, boolean shared) 
		...
		if ((type.equals("basicmodel")) || (type.equals("primitive"))) { 
			...
			switch (shapeType) {
				case 1 : result = Primitive.createQuad(dimensions.getX(), dimensions.getY()); break;
				case 2 : result = Primitive.createBox(dimensions); break;
				case 3 : result = Primitive.createSphere(dimensions.getX(), 18); break;
				case 4 : result = Primitive.createCylinder(dimensions.getX(), dimensions.getY(), 5, 5); break;
	...
	private void printSceneGraphNode(StringBuilder sb, SceneGraphNode node) {
		...
		sb.append(printTag("scale", "" + node.getTransform().getScale().getX())); // Uniform scaling
		if (type.equals("model")) {
