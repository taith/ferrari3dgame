...
import com.dennisbijlsma.core3d.renderer.jme.JMESceneGraphNode;
...
public class F3dLoader {
	...
	public F3dFile load(String location,boolean local,boolean share) throws Exception {
		...
		Document document=getXML(Resources.getResourceFile(location));
		...
		file.setNode(new Model(file.getName()));
		
		for (Element i : XMLParser.getChildNodes(scenegraphNode)) {
			file.getNode().addChild(parseSceneGraphNode(i,prefix));
		}
	...
	private SceneGraphNode parseSceneGraphNode(Element node,String prefix) throws IOException {
		...
		if (type.equals("model")) {
			...
			result=getModel(Resources.getResourceFile(prefix+modelURL));
			result.setName(id);
	...
	private void printSceneGraphNode(PrintWriter writer,SceneGraphNode node) {
		...
		if (type.equals("model")) {
			...
			writer.println(printTag("url",new File(model.getGeometryResource().toString()).getName(),3));
		}
	 ...
	 * @throws IOException when the file could not be converted.
	 */
	
	public void convertModel(File source,File target) throws IOException {
		
		Model model=new Model("source");
		model.setGeometry(source.toURI().toURL(),source.getParentFile().toURI().toURL());
		JMonkeyEngineRenderer renderer=((JMonkeyEngineRenderer) RenderSystem.getRenderer());
		renderer.saveModel((JMESceneGraphNode) model.getBinding(),target);
	}
	...
	private Model getModel(ResourceFile resource) throws IOException {
	
		URL key=resource.getURL();
		Model model=new Model(key.toString(),key,key);
		return model;
	}
	...
	private Image getTexture(URL key) {
		...
		if (!textureCache.containsKey(key.toString())) {
			try {
				InputStream stream=key.openStream();
				Image image=DataLoader.loadImage(stream);
				stream.close();
				textureCache.put(key.toString(),image);
			} catch (IOException e) {
	...
	private Document getXML(ResourceFile resource) {
