...
public class EditorGUI extends FormPanel {
	...
	public EditorGUI(Preview preview) {
		...
		MacAdapter.setComponentProperty(nodesList,MacAdapter.QUAQUA_LIST_STRIPED);
		...
		MacAdapter.setComponentProperty(addNodeButton,MacAdapter.AQUA_BUTTON_SQUARE);
		MacAdapter.setComponentProperty(addNodeButton,MacAdapter.QUAQUA_BUTTON_SQUARE);
		...
		MacAdapter.setComponentProperty(removeNodeButton,MacAdapter.AQUA_BUTTON_SQUARE);
		MacAdapter.setComponentProperty(removeNodeButton,MacAdapter.QUAQUA_BUTTON_SQUARE);
		...
		tNodeType=new JComboBox(new String[]{"SceneGraphNode","Model","BasicModel","Billboard"});
		...
		MacAdapter.setComponentProperty(carInfoList,MacAdapter.QUAQUA_LIST_STRIPED);
	...
	private void refreshNodeGUI() {
		...
		if (selectedNode!=null) {
			...
			if (selectedNode instanceof BasicModel) { tNodeType.setSelectedIndex(2); }
			if (selectedNode instanceof Billboard) { tNodeType.setSelectedIndex(3); }
			Matrix3D transform=new Matrix3D(selectedNode.getTransform());
			...
			tNodeTransform[6].setText(""+transform.getScale());
		} else {
	...
	private void addChildNode() {
		...
		if (nodeType.equals("Billboard")) {
			...
			BasicModel quad=new BasicModel("billboardQuad",BasicModel.TYPE_QUAD,new Vector3D(1f,1f,0f));
			quad.setMaterial(new Material(texture));
	...
	private void transformNode() {
		...
		Matrix3D transform=getSelectedNode().getTransform();
	...
	protected void cloneNode(SceneGraphNode node) {
		...
		if (node instanceof Model) { 
			...
			((Model) clone).setMaterial(((Model) node).getMaterial());
		}
